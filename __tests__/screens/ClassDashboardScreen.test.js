...
test('suppression élève demande confirmation', async () => {
  const { getByTestId, getByText } = render(
    <ClassDashboardScreen
      route={{ params: { classRow: { id: 'c1', name: '4e Rose' } } }}
      navigation={{ navigate: jest.fn() }}
    />,
  );
  expect(getByTestId('class-dashboard-list').props.contentContainerStyle).toEqual(
...
test('modifier un élève via le menu', async () => {
  ...
  await waitFor(() =>
    expect(updateStudent).toHaveBeenCalledWith(student, {
