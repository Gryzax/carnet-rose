...
import { enableLocalMode, signInWithApple, signInWithGoogle } from '../services/auth/authService';
...
export const LoginScreen = ({ onAuthenticated }) => {
  ...
  const runAuth = async (provider) => {
    ...
    setLoading('');
    if (result.user) {
      onAuthenticated?.({ user: result.user, localMode: false });
      return;
    }
  ...
  const continueLocal = async () => {
    ...
    setLoading('');
    onAuthenticated?.({ user: null, localMode: true });
  };
    ...
    <SafeAreaView style={[styles.screen, { backgroundColor: palette.canvas }]}>
      <View style={styles.wrap}>
        <View style={[styles.card, compact && styles.cardCompact, { backgroundColor: palette.card, borderColor: palette.border }]}>
          ...
          <Text style={[styles.title, { color: colors.deepPink }]}>Carnet Rose</Text>
          <Text style={[styles.subtitle, { color: palette.text }]}>Suivi hors ligne des élèves</Text>
          <Text style={[styles.reassure, { color: palette.muted }]}>Vos données restent disponibles hors ligne.</Text>
          {!supabaseReady && (
            <View testID="local-mode-notice" style={[styles.notice, { backgroundColor: palette.notice, borderColor: palette.border }]}>
              <Text style={[styles.noticeTitle, { color: colors.deepPink }]}>Mode local disponible</Text>
              <Text style={[styles.noticeText, { color: palette.muted }]}>Supabase n'est pas encore configuré.</Text>
            </View>
          )}
          ...
          {!supabaseReady && (
            <LoginButton testID="continue-local" disabled={Boolean(loading)} onPress={continueLocal} icon="phone-portrait-outline" palette={palette}>
              {loading === 'local' ? 'Ouverture...' : 'Continuer en mode local'}
            </LoginButton>
          )}
          {!!loading && <ActivityIndicator color={colors.deepPink} />}
          {!!message && <Text testID="login-message" style={[styles.message, { color: colors.deepPink }]}>{message}</Text>}
        </View>
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: 'PatrickHand_400Regular', fontSize: 42, lineHeight: 48, textAlign: 'center' },
  ...
  noticeTitle: { fontFamily: 'PatrickHand_400Regular', fontSize: 21 },
  noticeText: { fontFamily: 'PatrickHand_400Regular', fontSize: 18 },
  ...
  message: { fontFamily: 'PatrickHand_400Regular', fontSize: 18, lineHeight: 23, textAlign: 'center' }
});
